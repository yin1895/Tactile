...
```txt
帮我配置这个 skill：https://github.com/yliust/Tactile
```

如果你使用的工作流会调用 LLM，请按需要配置 API 环境：
...
第一版以 skill 的形式接入 Codex。在早期测试中，对于无障碍适配较好的 macOS 应用，accessibility-first 流程可以显著减少截图推理和坐标重试。
